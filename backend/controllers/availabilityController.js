...
exports.getAvailableSlots = async (req, res) => {
  try {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(dateString)) {
      return res.status(400).json({ success: false, error: 'التاريخ غير صالح' });
    }

    if (!serviceName) {
      return res.status(400).json({ success: false, error: 'يرجى اختيار خدمة' });
    }
    ...
    if ([workStart, workEnd, startOfDay, endOfDay].some((value) => Number.isNaN(value.getTime()))) {
      return res.status(500).json({
        success: false,
        error: 'إعدادات ساعات العمل غير صالحة'
      });
    }
    ...
    return res.json({
      ...
      businessNow: new Intl.DateTimeFormat('ar', {
        timeZone: BUSINESS_TIME_ZONE,
        dateStyle: 'short',
        timeStyle: 'medium'
      }).format(now)
    ...
    return res.status(500).json({
      success: false,
      error: 'حدث خطأ أثناء فحص الأوقات المتاحة'
    });
  }
};
